...
AFRAME.registerComponent('multisrc', {
  ...
  makeMaterials: function () {
    ...
    for (i = 0; i < this.sides; i++) {
      ...
      this.materials.push(
        new THREE.MeshStandardMaterial({ name: 'material-' + i + '' })
      );
    } //end for images loop
  }, //end makeMaterials
  ...
  addSrcs: function () {
    ...
    for (i = 0; i < this.data.srcs.length; i++) {
      ...
      if (
        fullasset.includes('.jpg') ||
        fullasset.includes('.jpeg') ||
        fullasset.includes('.png')
      ) {
  ...
  reduceMaterialChangedThrottle: function (throttle) {
    ...
    material.throttledEmitComponentChanged = AFRAME.utils.throttle(
      function emitChange() {
